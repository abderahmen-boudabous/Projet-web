<?php
	...
	class MovieC {
		...
		function affichermovies22(){
			$str=$_POST["search"];
			$sql22="SELECT * FROM movies WHERE title='$str' ";
			$db = config::getConnexion();
			try{
				$liste22 = $db->query($sql22);
				return $liste22;
			}
			catch(Exception $e){
				die('Erreur:'. $e->getMeesage());
		...
		function ajoutermovie($movie){
			$sql="INSERT INTO movies (idMovie, title, director, region, genre, rating, poster) 
		...
		function modifierrating($idMovie){
			try {
				$db = config::getConnexion();
				$query = $db->prepare(
					'UPDATE movies SET 
